...
def criar_agenda_padrao():
    ...
    for dia in dias:
        ...
        while hora <= fim:
            ...
            cursor.execute("SELECT * FROM horarios WHERE dia=? AND hora=?" if not USE_POSTGRES else
                           "SELECT * FROM horarios WHERE dia=%s AND hora=%s",
                           (dia, hora_str))
            if cursor.fetchone() is None:
                cursor.execute("INSERT INTO horarios (dia, hora, cliente) VALUES (?, ?, ?)" if not USE_POSTGRES else
...
def get_agenda(dia):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, hora, cliente FROM horarios WHERE dia=? ORDER BY hora" if not USE_POSTGRES else
...
def marcar_horario(id_horario, cliente):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE horarios SET cliente=? WHERE id=?" if not USE_POSTGRES else
...
def cancelar_horario(id_horario):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE horarios SET cliente=NULL WHERE id=?" if not USE_POSTGRES else
...
def buscar_cliente(nome):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT dia, hora, cliente FROM horarios WHERE cliente LIKE ?" if not USE_POSTGRES else
